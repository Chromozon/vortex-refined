...
void FindMonsterSpot (edict_t *self)
{
	...
	// there is a 20% chance that a boss will spawn in FFA
	// if the server is one quarter or more full
	
	if (ffa->value && !BossExists() && (self->num_sentries < 1) && (players >= 0.25*maxclients->value))
	{
		int chance = 20;
